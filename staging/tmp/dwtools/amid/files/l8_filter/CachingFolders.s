...
var _global = _global_;
var _ = _global_.wTools;
_.assert( !_.FileFilter.CachingFolders );

// _.FileFilter = _.FileFilter || Object.create( null );
// if( _.FileFilter.CachingFolders )
// return;
...
function _select( path, mode, value )
{
  ...
  if( usingDelete )
  {
    var dir = self.path.resolve( self.path.dir( path ) );
    dir = self.path.relative( self.rootPath, dir );
    if( dir === '.' )
    dir = '';
    var fileName = self.path.name({ path : path, withExtension : 1 });
  }
